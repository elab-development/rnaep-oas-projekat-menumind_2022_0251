// Small Kafka helper shared (by copy) across MenuMind services.
// Producers are fire-and-forget: Kafka being down must never break an HTTP request.
...
export function createKafkaClient(clientId: string) {
  ...
  // Subscribes to topics and processes messages with `handler(topic, payload)`.
  // Retries the initial connection forever so services can start before Kafka.
